...
Task Input Parameters allow you to add additional information to Task resources.
...
way the [dsf-task-base](https://github.com/datasharingframework/dsf/blob/main/dsf-fhir/dsf-fhir-validation/src/main/resources/fhir/StructureDefinition/dsf-task-base-1.0.0.xml) adds slices to the original [FHIR Task](https://www.hl7.org/fhir/R4/task.html) resource. Notice that this also requires creating a CodeSystem and
including it in a ValueSet to be able to use it in the Task resource.

#### Task Output Parameters

Task Output Parameters server a similar purpose to Task Input Parameters. They add additional information to a Task resource, but for a different context.
While Task Input Parameters provide additional information which is required to process the Task, Task Output Parameters provide information on the Task's completion.
